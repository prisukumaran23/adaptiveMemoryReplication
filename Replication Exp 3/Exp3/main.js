    ...
    set_score = function(SCORE_HIGH,SCORE_LOW){
        if(SCORE_HIGH>=27 && SCORE_LOW>=13){
            bonus = 5.25}
        else if(SCORE_HIGH>=13){bonus = 5.00}
        else if(SCORE_LOW>=13){bonus = 0.25}
        else{bonus = 0}
        return bonus;
        };
    ...
    var instructions3 = {
        ...
        "</div>"+
        "<div class='space'></div></p><p>Phase 2 will take no more than 12 minutes."+
    ...
    var instructions_break_2 = {
      ...
      html +=  "</p><style='font-size: 30px;color: #006600;'><p>Your bonuses will be calculated and paid after completing day 2."+
      "</p><p><b>Press J</b> when you are ready to continue to the final part of the study.</p>"+
    ...
    end_timeline.push({
        type: 'call-function',
        func: function(){
          saveData("ReplicationPatil_Day1", jsPsych.data.get().json());
        }
        });
